...
class ultraimportTests(unittest.TestCase):
    ...
    def test_stackoverflow_74632397(self):
        file_path = "examples/stackoverflow-74632397/main.py"
        ret = self.exec(file_path)
        self.assertEqual(ret.returncode, 0, f'Running {file_path} did return with an error: {ret}')
        # Check that the Stuff class is imported correctly
        self.assertIn(b"<class 'autogenerated.file1.Stuff'>", ret.stdout)

    def test_parameter_combinations_typed_imports(self):
        """Test parameter combinations using typed-imports example"""
        file_path = "examples/typed-imports/run.py"
        ret = self.exec(file_path)
        self.assertEqual(ret.returncode, 0, f'Running {file_path} did return with an error: {ret}')

        # Check that myfunction is correctly imported as callable
        self.assertIn(b"<function myfunction at", ret.stdout)
        self.assertIn(b"<class 'function'>", ret.stdout)

        # Check that mystring is correctly imported as string
        self.assertIn(b"Hello world <class 'str'>", ret.stdout)

        # Check that type mismatch is detected
        self.assertIn(b"Type mismatch detected", ret.stdout)
        self.assertIn(b"Import type mismatch, expected 'myint' to be of type", ret.stdout)

    def test_dependency_injection_with_namespace(self):
        """Test add_to_ns parameter using dependency-injection example"""
        file_path = "examples/dependency-injection/run.py"
        ret = self.exec(file_path)
        self.assertEqual(ret.returncode, 0, f'Running {file_path} did return with an error: {ret}')

        # Check that objects were properly added to namespace (as visible by output)
        self.assertIn(b"injected logger: worker.py is doing some work", ret.stdout)
